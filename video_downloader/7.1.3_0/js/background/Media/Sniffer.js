...
//					if( tabInfo.url.toLowerCase().match( new RegExp(SKIP_SNIFFER_SIGNS[i],'g') )  )   return null;
				}
				
/*
				if ( /twitch.tv\/([^.]*)/i.test(tabInfo.url.toLowerCase()) )		return null;
				if ( /periscope.tv\/([^.]*)/i.test(tabInfo.url.toLowerCase()) )		return null;
				if ( /break.com\/video\/([^.]*)/i.test(tabInfo.url.toLowerCase()) )		return null;
				if ( /http:\/\/(www\.)?dailymotion(\.co)?\.([^\.\/]+)\//i.test(tabInfo.url.toLowerCase()) )		return null;
				if ( /www\.facebook\.com\/(.*)\/videos/i.test(tabInfo.url.toLowerCase()) )		return null;
