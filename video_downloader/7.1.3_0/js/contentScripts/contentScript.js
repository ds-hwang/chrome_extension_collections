(function(){
		...
		function buildMediaListDM( media, parent ){
			...
			if( media.length == 0 )
			{
				...
				span.addEventListener("click", function(){
								...
								//port.postMessage( {	action: "fvdSetting"	} );	
				}, true);
	...
	function iconVKontakte( media, port ) {
		...
		var elem_list = document.getElementById("audios_list");
		if 	(elem_list == null) elem_list = document.getElementById("initial_list");
		if 	(elem_list == null) elem_list = document.getElementById("results");
		if 	(elem_list == null) elem_list = document.getElementById("profile_audios");
		
		var elems_input = elem_list.getElementsByTagName("input");	
		for (var i=0; i<elems_input.length; i++)
		{
			var url_input = elems_input[i].value;
			var p = url_input.split(",");
			url_input = p[0];
			
			var id_input = elems_input[i].id;
